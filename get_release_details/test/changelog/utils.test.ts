...
const CURRENT_VERSION = '0.0.40'
const CHANGELOGS_WITH_DUPLICATED = `
...
describe('RemoveChangelogDuplicates', () => {
  test('it should remove all duplicates from a changelog', (done) => {
    ...
    const index = CHANGELOGS_WITH_DUPLICATED.indexOf(CURRENT_VERSION)
    const oldChangelog = CHANGELOGS_WITH_DUPLICATED.substring(index)

    const changelogs = removeDuplicates(CHANGELOGS_WITH_DUPLICATED, CURRENT_VERSION)
    ...
    for (const line of `${changelogs}${oldChangelog}`.split('\n')) {
      ...
      if (lines[line]) {
        return done(new Error(`Duplicate found here: ${line}`))
      } else {
        lines[line] = true
      }
    }
