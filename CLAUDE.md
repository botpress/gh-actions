...
The six legacy `node12` actions (`rename_binaries`, `get_release_details`, `fetch_release_changelogs`, `close_pull_request_issues`, `set_ssh_key`, `extract_info`) were deleted. With them, the entire Node toolchain became dead — `jest.config.ts`, `.github/workflows/check-dist.yml`, `.github/workflows/test.yml`, and `.github/workflows/codestyle.yml` are gone too, and `package.json` no longer lists `workspaces` / `test`-related devDeps. `.oxfmtrc.json` and `tsconfig.json` remain in place so a new TypeScript action can be added back without re-bootstrapping; if you do, add `oxlint` + `.oxlintrc.json`, restore `codestyle.yml`, and add the package to a `workspaces` array.
...
There are currently no Node actions in this repo — every action is a `composite` shelling out to `bash`. To work on one, edit `action.yml` directly and ship a tag. There is no build step.

Package manager is **pnpm 10.29.3** (pinned via `packageManager` in `package.json`). Formatting runs through **oxfmt** (`pnpm check:format` / `pnpm fix:format`, aliased as `pnpm check` / `pnpm fix`). See `.oxfmtrc.json`. There is no linter wired up — the repo has no TS code today; if you reintroduce one, add `oxlint` + `.oxlintrc.json` (see `repos/sauron` for a reference setup).

If you reintroduce a Node action: add it to a `workspaces` array in `package.json`, restore the jest/ts-jest devDeps + a `test` script if needed, restore `.github/workflows/codestyle.yml` (wire it to `pnpm check`), and commit the bundled `dist/index.js` (consumers reference it directly via `runs.main`). Use `node20`, **not** `node12`/`node16` — both are deprecated by GitHub.
...
- **Node engine pinned to 22.x** in root `package.json` (matches `.nvmrc` → `22.15.0`), but composite-action consumers run on the GitHub-hosted runner's node — version varies per action (`publish-if-not-exists` requires `24.x`, `publish-private-package` uses `24`).
